...
class SeguidorEmissorTBJ(Frame):
	def __init__(self, raiz):
		...
		# Ordem Entradas
		self.t_entradas = ["Rb", "Re", "ro", "Beta", "Vcc"]
		# Ordem Saidas
	...
	def calcularSaidas(self):
		# Ib
		try:
			self.dados["Ib"]["val"] = (self.dados["Vcc"]["val"]-0.7)/(self.dados["Rb"]["val"]+(self.dados["Re"]["val"]*(self.dados["Beta"]["val"]+1)))
		...
		# Avnl
		try:
			self.dados["Avnl"]["val"] = self.dados["Re"]["val"]/(self.dados["Re"]["val"]+self.dados["re"]["val"])
		...
		# Zb
		try:
			self.dados["Zb"]["val"] = self.dados["Beta"]["val"]*(self.dados["Re"]["val"]+self.dados["re"]["val"])
			self.var_saidas[self.t_saidas.index("Zb")].set(f'{self.dados["Zb"]["val"]:e}')
		except:
			self.dados["Zb"]["val"] = "-"
			self.var_saidas[self.t_saidas.index("Zb")].set(f'{self.dados["Zb"]["val"]}')
		# Zi
		try:
			self.dados["Zi"]["val"] = paralelo(self.dados["Rb"]["val"],self.dados["Zb"]["val"])
		...
		# Zo
		try:
			self.dados["Zo"]["val"] = paralelo(paralelo(self.dados["re"]["val"],self.dados["ro"]["val"]),paralelo(self.dados["Re"]["val"],self.dados["Beta"]["val"]*self.dados["re"]["val"]))
