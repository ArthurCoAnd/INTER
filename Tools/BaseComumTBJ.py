...
class BaseComumTBJ(Frame):
	def __init__(self, raiz):
		...
		# Ordem Entradas
		self.t_entradas = ["Rc", "Re", "ro", "Alfa", "Vee"]
		# Ordem Saidas
		self.t_saidas =  ["Ie", "re", "Avnl", "Zi", "Zo"]
	...
	def calcularSaidas(self):
		# Ie
		try:
			self.dados["Ie"]["val"] = (self.dados["Vee"]["val"]-0.7)/self.dados["Re"]["val"]
		...
		# Avnl
		try:
			self.dados["Avnl"]["val"] = -paralelo(self.dados["ro"]["val"],self.dados["Rc"]["val"])*self.dados["Alfa"]["val"]/self.dados["re"]["val"]
		...
		# Zi
		try:
			self.dados["Zi"]["val"] = paralelo(self.dados["Re"]["val"],self.dados["re"]["val"])
